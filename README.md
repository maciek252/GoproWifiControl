...
Connect the IR port from STorM32 to nodes GND and D5 (GND-GND and IR-D5). Power the board with 5V (I use a 12V->5V step down converter). Add a resistor and/or a LED between these wires, so that there are no unstable states. I have 17 kohms, as far as I can decode the colors (red, purple, orange, gold). You can also test by connecting D5 between 3.3V (shots photos) and GND.
